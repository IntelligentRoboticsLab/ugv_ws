...
class ServiceHandler(Node):
    def __init__(self, group):
        ...
        self.kill_node_service = self.create_service(ManageNode, 'vizanti/node/kill', self.node_kill, callback_group=group)
    ...
    def node_start(self, req, res):
        try:
            ...
            self.get_logger().info("Starting "+str(req.node))

            res.success = True
            res.message = f'Started {req.node}'
        except Exception as e:
            res.success = False
            res.message = str(e)
        return res
    ...
    def run_shellcommand(self, *args):
        ...
        args = sum([(arg if isinstance(arg, list) else [arg]) for arg in args], [])
        print(args)
    ...
    def load_map(self, req, res):
        ...
        try:
            ...
            #process = subprocess.Popen(["ros2", "run", "nav2_map_server", "map_server", "--ros-args", "--params-file", file_path], stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
            #flags = fcntl.fcntl(process.stdout, fcntl.F_GETFL)
            #fcntl.fcntl(process.stdout, fcntl.F_SETFL, flags | os.O_NONBLOCK)
            process = self.run_shellcommand(["ros2", "run", "nav2_map_server", "map_server", "--ros-args", "--params-file", file_path])
    ...
    def save_map(self, req, res):
        ...
        try:
            ...
            process = subprocess.Popen(["ros2", "run", "nav2_map_server", "map_saver_cli", "-f", file_path], stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
